document.addEventListener('DOMContentLoaded', () => {
    ...
    const itemDB = {
        'shortSword': { name: 'Short Sword', type: 'weapon', slot: 'rightHand', stats: { str: 3 }, image: 'img/shortsword.webp' },
        'woodenBuckler': { name: 'Wooden Buckler', type: 'shield', slot: 'leftHand', stats: { end: 2 }, image: 'img/woodenbuckler.webp' },
        'longSword': { name: 'Long Sword', type: 'weapon', slot: 'rightHand', stats: { str: 5 }, image: 'img/longsword.webp' },
        ...
        'dagger': { name: 'Dagger', type: 'weapon', slot: 'rightHand', stats: { str: 1, spd: 2 }, image: 'img/dagger.webp' },
        'spear': { name: 'Spear', type: 'weapon', slot: 'rightHand', stats: { str: 4 }, image: 'img/spear.webp' },
        'woodenStaff': { name: 'Wooden Staff', type: 'weapon', slot: 'rightHand', stats: { mag: 3 }, image: 'img/staff.jpeg' },
        'ironStaff': { name: 'Iron Staff', type: 'weapon', slot: 'rightHand', stats: { mag: 5 }, image: 'path/to/iron_staff.png' },
        'knowledge': { name: 'Sword of Enlightement', type: 'weapon', slot: 'rightHand', stats: { str: 10, spd: 2 }, image: 'path/to/iron_staff.png' },
    ...
    const skillDB = {
        ...
        'ChainofTorment': {name: 'Chain of Torment', cost: 50, type: 'magic', damage: 35}
    };
    ...
    const classDB = {
        ...
        'Mercenary': {
            ...
            startEquip: ['dagger', 'dagger'], 
            skills: [],
            image: 'img/hero.gif' 
        },
        'Priest': {
            baseStats: { str: 6, end: 5, spd: 5, mag: 15 },
            startEquip: ['woodenStaff'],
            skills: ['fireball', 'minorHeal', 'ChainofTorment'],
            image: 'img/hero.gif' 
        }
    };
    ...
    const enemyDB = {
        'tier1': [
            { name: 'Goblin', tier: 1, hp: 30, stats: { str: 5, end: 2, spd: 5 }, xp: 10, image: 'img/goblin1.gif', drops: [{ id: 'goblinEar', chance: 0.5 }, { id: 'goblinEye', chance: 0.2 }] },
            { name: 'Skeleton', tier: 1, hp: 25, stats: { str: 4, end: 1, spd: 3 }, xp: 8, image: 'img/skeleton1.webp', drops: [{ id: 'skeletonBone', chance: 0.5 }] }
        ],
        'tier2': [
            { name: 'Hobgoblin', tier: 2, hp: 60, stats: { str: 8, end: 4, spd: 4 }, xp: 25, image: 'img/goblin2.gif', drops: [{ id: 'goblinEar', chance: 0.8 }, { id: 'goblinEye', chance: 0.4 }] },
        ...
        'tier3': [
            { name: 'Goblin Raider', tier: 3, hp: 100, stats: { str: 12, end: 6, spd: 8 }, xp: 50, image: 'img/goblin3.gif', drops: [{ id: 'goblinEar', chance: 1.0 }, { id: 'goblinEye', chance: 0.5 }, { id: 'spear', chance: 0.1 }] },
        ...
        'tier4': [
            { name: 'The Enlightened One', tier: 4, hp: 250, stats: { str: 14, end: 6, spd: 8 }, xp: 50, image: 'img/yellowking.webp', drops: [{ id: 'knowledge', chance: 10.0 }] },
            { name: 'The Tormented One', tier: 4, hp: 280, stats: { str: 17, end: 4, spd: 6, mag: 10 }, xp: 60, image: 'path/to/skeleton_mage.png', drops: [{ id: 'skeletonBone', chance: 1.0 }, { id: 'woodenStaff', chance: 0.1 }] }
        ]
    };
    ...
    // *** FUNGSI SPAWN BARU DENGAN PERSENTASE ***
    function startNextEncounter() {
        // Logika Probabilitas (Dadu 0-100)
        const chance = Math.random() * 100;
        
        if (player.level <= 2) {
            // Level 1-2: 80% Tier 1, 20% Tier 2
            if (chance < 80) currentEncounterTier = 1;
            else currentEncounterTier = 2;
        } else if (player.level <= 5) {
            // Level 3-5: 30% Tier 1, 50% Tier 2, 20% Tier 3
            if (chance < 30) currentEncounterTier = 1;
            else if (chance < 80) currentEncounterTier = 2; // 30 + 50 = 80
            else currentEncounterTier = 3;
        } else if (player.level <= 10) {
            // Level 6+: 10% Tier 1, 40% Tier 2, 50% Tier 3
            if (chance < 10) currentEncounterTier = 1;
            else if (chance < 50) currentEncounterTier = 2; // 10 + 40 = 50
            else currentEncounterTier = 3;
        }
          else {
            if (chance < 9) currentEncounterTier = 2;
            else if (chance < 99) currentEncounterTier = 3;
            else currentEncounterTier = 4;
          }
        ...
        clearCombatLog();
        addCombatLog(`Seekor ${currentEnemy.name} (Tier ${currentEncounterTier}) muncul!`);
    ...
    function enemyTurn() {
        ...
        // Damage Reduction: 1 END = 0.5% reduction (max 75%)
    ...
    function updateCombatUI() {
        ...
        if (player && player.name) {
            ...
            // Gambar Hero
            if (player.image) {
                document.getElementById('player-image').innerHTML = `<img src="${player.image}" alt="${player.class}">`;
            } else {
